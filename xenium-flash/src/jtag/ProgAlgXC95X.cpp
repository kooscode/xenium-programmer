...
Copyright (C) Copyright 2001 Nahitafu,Naitou Ryuji
...
Koos du Preez [kdupreez@hotmail.com] Jan 1 2019 
    Accepts device ID and figures out DRLength
*/
...
ProgAlgXC95X::ProgAlgXC95X(Jtag &j, const uint32_t device_id)
{
  jtag=&j;

  int dr_type = (device_id & 0x000ff000) >> 13;
  switch (dr_type)
   {
   case  1: DRegLength = 2;
     break;
...
int ProgAlgXC95X::array_program(JedecFile &file)
{
  return flow_array_program(file);
}
